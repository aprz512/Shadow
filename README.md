...
### 环境准备
建议直接使用支持 JDK 17 和 Gradle 9.3 的 Android Studio 稳定版打开工程。
低版本的 Android Studio 可能因为 Gradle 版本过高而无法正常打开项目。

当前仓库的主构建链路以 JDK 17 为基线，CI 默认也使用 JDK 17 执行 `build`、`test` 和 AGP 兼容性测试。
其中 `sample/maven` 目录仍保留了基于 JDK 11 的消费侧构建校验，用来验证已发布 SDK 的 Maven 接入场景。
...
* 当前自动化验证的 AGP 兼容性基线为 `AGP 9.x + Gradle 9.3.1 + JDK 17`，对应脚本见 [test_JDK17.sh](projects/test/gradle-plugin-agp-compat-test/test_JDK17.sh)。
* 当前仓库已经不再兼容 AGP 7.x 和 8.x。此次迁移直接删除了旧版 AGP 兼容分支，并将维护重点收敛到 AGP 9.x，避免继续在新旧 API 之间维持复杂的条件分支。
* 如果你的宿主工程仍停留在 AGP 7.x 或 8.x，建议固定到旧版 Shadow 提交，或基于旧版本自行维护分支；不要直接拿当前 `master` 回退适配旧版 AGP。
* `projects/test/gradle-plugin-agp-compat-test/README.md` 记录了黑盒兼容性测试的入口和约束，便于单独验证 Gradle 插件侧行为。
